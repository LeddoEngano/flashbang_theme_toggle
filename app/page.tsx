  ...
  const handleClick = useCallback(() => {
    ...
    if (isDark) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setIsFlashing(false), 4500); // total flash duration
      }, 50);
    } else {
      setTheme(true);
    }
...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen w-full grid place-items-center px-6 overflow-hidden">
      ...
      <div className="flex flex-col items-center text-center gap-8 md:gap-10 max-w-2xl">
        ...
        <p className="text-xs text-zinc-800 dark:text-zinc-400">Click the grenade to toggle the theme</p>

        <div className="mt-2 flex items-center gap-3 text-sm text-zinc-900 dark:text-zinc-300">
          <Image
            src="/leddo.webp"
        ...
        <div className="mt-3 flex flex-wrap items-center justify-center gap-3">
          <a
            ...
            className="inline-flex items-center gap-2 rounded-full border border-zinc-200 dark:border-zinc-700 px-3 py-1.5 text-sm text-zinc-800 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors"
            aria-label="Perfil no X (Twitter) de @leddo_401"
          >
            <FaXTwitter className="h-4 w-4 shrink-0" aria-hidden />
            <span className="opacity-80">@leddo_401</span>
          </a>
          <a
          ...
          >
            ...
            <span className="opacity-80">@leddo_</span>
          </a>
        </div>
      </div>
      {showFlash && (
